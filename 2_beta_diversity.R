...
rda.form <- as.formula("dist ~ Time*Group + Condition(Plate + Country)")
...
for(i.ps in names(pss.ls)) {
  ...
  for(i.lvl in c("css_ASV", "css_Genus")) {
    ...
    for (i.dist in used.dist) {
      ...
      if(i.dist == "jaccard") {
        ...
        comb.dist[[i.ps]][[i.lvl]][[i.dist]] <- distance(ps.inst, 
                                                         i.dist, 
                                                         binary = TRUE)
  ...
  for(i.lvl in c("css_ASV", "css_Genus")) {
    ...
    for (i.dist in used.dist) {
      ...
      centr.df <- rda.p.auto$data[[4]] %>%
        mutate(Centroids = gsub(var.use["Group"], "", label))
      
      vectors.df <- rda.p.auto$data[[3]] %>%
        mutate(Group = gsub(var.use["Group"], "", label))
      ...
      beta <- betadisper(dist,
                         group = meta.ls[[i.gr]][[var.use["Group"]]])
